  ...
  return (
    <section className="bg-[#ebe5d8] px-[clamp(24px,9vw,140px)] py-25 max-md:py-18">
      <Reveal>
        ...
        <div className="grid grid-cols-3 border-y border-[#18392f]/15 max-md:grid-cols-1">
          {data.map(({ icon: Icon, title, copy }, i) => (
            <article
              key={title}
              className="relative border-r border-[#18392f]/15 px-9 py-10 last:border-r-0 max-md:border-r-0 max-md:border-b max-md:px-5 max-md:py-8"
            >
              <span className="absolute top-5 right-6 font-serif text-2xl text-[#9b9e91]">
                0{i + 1}
              </span>
              <Icon
                size={28}
                strokeWidth={1.35}
                className="mb-6 text-[#e56d3d]"
              />
