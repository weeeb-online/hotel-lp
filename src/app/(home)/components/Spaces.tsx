    ...
    >
      <Reveal>
        ...
        <div className="mb-12 flex items-end justify-between max-md:block">
          <h2 className="m-0 font-serif text-[clamp(52px,6vw,82px)] leading-[.85] tracking-[-.035em]">
            Seu espaço para
            <br />
            <em>sentir-se bem.</em>
          </h2>
        ...
        <div
          role="tablist"
          className="flex overflow-x-auto border-b border-[#18392f]/15"
        >
          {spaces.map((x, i) => (
            <button
              key={x.name}
              onClick={() => setA(i)}
              className={`shrink-0 border-0 border-b-2 bg-transparent px-6 py-4 text-[11px] first:pl-0 ${i === a ? "border-[#e56d3d] font-extrabold text-[#18392f]" : "border-transparent text-[#71817a]"}`}
            >
              {x.name}
            </button>
          ))}
        ...
        <div className="mt-7 grid min-h-100 grid-cols-[1.65fr_1fr] bg-white max-md:grid-cols-1">
          ...
          <div className="flex flex-col items-start justify-center px-12 py-14 max-md:px-7 max-md:py-10">
            <small className="text-[9px] font-extrabold tracking-[.2em] text-[#e56d3d]">
              {s.type}
            </small>
            <h3 className="my-4 font-serif text-[43px]">{s.name}</h3>
            <p className="text-[13px] leading-6 text-[#64766f]">{s.desc}</p>
            <span className="mb-6 w-full border-t border-[#18392f]/15 pt-4 text-[10px] text-[#65766f]">
              {s.meta}
            </span>
